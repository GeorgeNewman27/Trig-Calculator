...
def findthirdangle(angles):
  for i in range (3):
    if (angles[i] == -1.0):
      ...
      for j in range (3):
        if (i != j):
          angles[i] = angles[i] - angles[j]
      print("Angle " + str(i + 1) +  " value calculated as " + str(angles[i]))
  return
...
def main():
  #[angle a, angle b, angle c, number of angles given, sum of angles given]
  angles = [-1.0, -1.0, -1.0, 0.0, 0.0]

  #[side a, side b, side c, number of sides given]
  sides =  [-1.0, -1.0, -1.0, 0.0]
  ...
  for j in range (3):
    ...
    if usera == "":
      ...
      #If entered value is a number, check to ensure it is within bounds
        ...
        if ((angles[j] <= 0.0) & (angles[j] != -1.0 )) | (angles[j] >= 180.0):
  ...
  if ((angles[3] == 3.0) & (angles[4] != 180.0)) | ((angles[3] < 3.0) & (angles[4] >= 180.0)):
